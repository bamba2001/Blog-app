...
public class PostsController {
	...
	public ModelAndView create(@Valid Post post, BindingResult bindingResult){
		...
		if( post.getTitle().isEmpty() ){
			bindingResult.rejectValue("title", "error.post", "Le titre ne peut pas être vide");
		}
		if( post.getBody().isEmpty() ){
			bindingResult.rejectValue("body", "error.post", "Le contenu ne peut pas être vide");
		}
		// Get author
		Authentication auth = SecurityContextHolder.getContext().getAuthentication();
		User user = this.userService.findByUserName(auth.getName());
		if( user==null ){
			bindingResult.rejectValue("author", "error.post", "L'auteur ne peut pas être nul");
		}
	...
	/**
	 * Obtenez une liste de tous les articles de la base de données, il devrait être capable de paginer et de trier
